...
int32 UserMysql::ServiceInfoSelect(std::string sid_str, DicValue* dic) {
  int32 err = 0;
  bool r = false;
  do {
    std::stringstream ss;
    ss << "call proc_ServiceInfoSelect('" << sid_str << "')";
...
int32 UserMysql::InvoiceInfoInsert(int64 oid, std::string title,
                                   std::string num, std::string comp,
                                   int64 type, std::string name,
                                   std::string phone, std::string area,
                                   std::string addr, std::string remarks,
                                   DicValue* dic) {
  int32 err = 0;
  bool r = false;
  do {
...
void UserMysql::CallServiceInfoSelect(void* param, Value* value) {
  ...
  if (num > 0) {
    ...
    while (rows = (*(MYSQL_ROW*)(engine->FetchRows())->proc)) {
      ...
      if (rows[5] != NULL)
        dict->SetBigInteger(L"is_servicing_", atoll(rows[5]));
      if (rows[6] != NULL)
